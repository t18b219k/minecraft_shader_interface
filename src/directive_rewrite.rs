...
impl DirectiveRewriter {
    pub fn new<Source: AsRef<str>>(source: Source) -> Self {
        ...
        let regex_extension = regex::Regex::new(
            r"#extension\s+(P?<ext>[a-zA-Z_][a-zA-Z0-9_]*?)\s*:\s*(P?<enable>.+?)",
        )
        .unwrap();
        Self {
    ...
    pub fn convert(&self) -> String {
        ...
        for line in self.source.lines() {
            if self.regex_version.captures(&self.source).is_none()
                && self.regex_extension.captures(&self.source).is_none()
            {
                lines.push(line);
            }
        }
